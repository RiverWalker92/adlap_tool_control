  ...
  : serial_(serial), logger_(logger), motor_(config)
{
  ...
  try
  {
    ...
    // send_duty_cycle({motor_.duty_cycle_percentage, motor_.duty_cycle_percentage, motor_.duty_cycle_percentage,
    // motor_.duty_cycle_percentage}, /*verbose=*/true); send_encoder_mode({motor_.encoder_mode, motor_.encoder_mode,
    // motor_.encoder_mode, motor_.encoder_mode}, /*verbose=*/true);
    for (int i = 0; i < 4; ++i)
    {
...
{
  std::string response = "targets " + std::to_string(m_array[0]) + ", " + std::to_string(m_array[1]) + ", " +
...
{
  std::string message = "duty " + std::to_string(duty_cycle_array[0]) + ", " + std::to_string(duty_cycle_array[1]) +
                        ", " + std::to_string(duty_cycle_array[2]) + ", " + std::to_string(duty_cycle_array[3]) + "\n";
  if (verbose)
  {
    std::string log_message = message.substr(0, message.size() - 1);
...
{
  std::string message = "encoder " + std::to_string(encoder_mode_array[0]) + ", " +
                        std::to_string(encoder_mode_array[1]) + ", " + std::to_string(encoder_mode_array[2]) + ", " +
...
void MotorController::send_motor_configuration(int motor_index, bool verbose)
{
  ...
  std::string message = "config " + std::to_string(motor_index) + ", " + std::to_string(motor_.duty_cycle_percentage) +
                        ", " +  // Min duty cycle (same as duty cycle for now, but could be different)
                        std::to_string(motor_.duty_cycle_percentage) +
                        ", " +  // Max duty cycle (same as duty cycle for now, but could be different)
                        std::to_string(motor_.max_current) + ", " + std::to_string(motor_.emergency_current) + ", " +
                        std::to_string(static_cast<int>(motor_.gear_ratio)) + ", " + std::to_string(motor_.magnets) +
                        ", " + std::to_string(motor_.encoder_mode) + ", " + std::to_string(reverse_int) + "\n";
  if (verbose)
  {
    std::string log_message = message.substr(0, message.size() - 1);
...
void MotorController::couple_sequence()
{
  ...
  for (int i : { 0, 1, 2, 3 })
  {
    ...
    do
    {
      ...
      if (!rclcpp::ok())
        throw std::runtime_error("Shutdown requested (rclcpp::ok() == false)");
    } while (!get_blocked()[i]);
...
void MotorController::setup_motors()
{
  ...
  setup_values[motor_nr] =
      (max_position + min_position) / 2 - lower_motor_start_offset;  // Add some extra to compensate for backlash
  ...
  setup_values = get_positions();
  if (hall_up1 > hall_down1)
    hall_down1 += get_pulses_per_rotation(false);
  setup_values[1] = (hall_up1 + hall_down1) / 2 + HALL2_OFFSET;  // The pin is not exactly in the middle
  if (hall_up2 > hall_down2)
    hall_down2 += get_pulses_per_rotation(false);
  ...
  RCLCPP_DEBUG(logger_, "Motor setup sequence completed with lower_motor_start_offset %d and gripper_start_offset %d",
               lower_motor_start_offset, gripper_start_offset);
}
