...
void Gearbox::setup_motors()
{
  ...
  // Temporarily set the duty cycle to a higher value to make sure the motor can move to the blocked position for
  // calibration.
  ...
  setup_values[motor_nr] =
      (max_position + min_position) / 2 - lower_motor_start_offset_;  // Add some extra to compensate for backlash
  ...
  RCLCPP_DEBUG(logger_, "Motor setup sequence completed with lower_motor_start_offset %d and gripper_start_offset %d",
               lower_motor_start_offset_, gripper_start_offset_);
}