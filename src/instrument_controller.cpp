...
InstrumentController::InstrumentController(Gearbox& gearbox, rclcpp::Logger logger) : gearbox(gearbox), logger_(logger)
{
}
...
void InstrumentController::set_euler_angles(double roll, double pitch, double yaw, double articulation, bool verbose)
{
  ...
  gearbox.motor_controller.send_motor_positions(
      calculate_motor_positions_from_euler_angles(smoothed_tip_rotation_, smoothed_pitch_, smoothed_yaw_,
...
int InstrumentController::get_motor2_value_for_angle(double radians, bool verbose)
{
  ...
  int starting_difference =
      gearbox.motor_controller.get_starting_positions()[2] -
      gearbox.motor_controller.get_starting_positions()[1];  // Starting difference between motor 2 and motor 1,
                                                             // compensated for the initial offset found in setup
  // int current_difference = gearbox.motor_controller.get_positions()[2] - gearbox.motor_controller.get_positions()[1]
  ...
  int current_difference = current_positions[2] - current_positions[1] - starting_difference -
  ...
  int deadband_threshold =
      static_cast<int>(std::round(gearbox.get_pulses_per_degree(1) * 2.5));  // deadband threshold in pulses
  if (verbose)
    RCLCPP_DEBUG(logger_,
                 "relative_difference: '%d', real_play: '%d', deadband_threshold: '%d', bend_play_compensation_: '%d', "
                 "current_difference: '%d', wanted_difference: '%d'",
                 relative_difference, real_play, deadband_threshold, bend_play_compensation_, current_difference,
                 wanted_difference);
  ...
  if (verbose)
    RCLCPP_DEBUG(logger_, "play_compensation_: '%d'", bend_play_compensation_);
                                                                                     ...
                                                                                     double articulation, bool verbose)
{
  ...
  int articulation_pulses =
      static_cast<int>(std::round(ARTICULATION_FACTOR * articulation * gearbox.get_pulses_per_rotation(3) / (TWO_PI)));
...
std::array<double, 4> InstrumentController::joint_angles_from_motors(const std::array<int, 4>& current_positions)
{
  ...
  double tip_rotation = (static_cast<double>(starting_positions[0] - current_positions[0]) * TWO_PI /
                         gearbox.get_pulses_per_rotation(0));  // Invert the roll calculation
  double articulation_angle = (static_cast<double>(current_positions[3] - starting_positions[3] - current_positions[0] +
                               ...
                               TWO_PI / gearbox.get_pulses_per_rotation(3)) /
