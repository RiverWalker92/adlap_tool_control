...
TRAINING_CONFIGURATIONS = {
    "motor_only": {
        "input_dir": DEFAULT_MOTOR_ONLY_DATA_DIR,
        "output_dir": DEFAULT_MOTOR_ONLY_RESULT_DIR,
    },
    "gearbox_only": {
    ...
    with open(file_path, "r") as f:
        for line in f:
            ...
            motor_commands_valid = (
                motor_commands is not None
                and len(motor_commands) >= 4
            )

            instrument_commands_valid = (
                instrument_commands is not None
                and len(instrument_commands) >= 4
            )

            if motor_commands_valid:
                command_source = "motor_relative"
                command_values = [float(x) for x in motor_commands[:4]]

            elif instrument_commands_valid:
                command_source = "instrument_angles"
                command_values = [float(x) for x in instrument_commands[:4]]

            else:
                # Geen bruikbaar commando; sample niet stilletjes als nulcommando opslaan.
                continue
...
def convert_instrument_angles_to_motor_targets(
    instrument_angles,
    gearbox_variant,
    instrument_config,
):
    """
    Convert one [dof1, dof2, dof3, dof4] command into four absolute
    commanded motor targets in encoder pulses.

    This function must call the same conversion used by tool_control_node.
    """
    raise NotImplementedError(
        "Connect this function to the DOF-to-motor conversion "
        "used by tool_control_node."
    )

def segment_to_arrays(
    segment,
    gearbox_variant,
    instrument_config,
):
    ...
    if len(command_sources) != 1:
        raise RuntimeError(
            f"Mixed command sources in segment: {command_sources}"
        )

    command_source = next(iter(command_sources))

    command_values = np.array(
        [row["command_values"] for row in rows],
        dtype=float,
    ).T
    ...
    if command_source == "motor_relative":
        # Motor-only runner: relative pulse commands.
        raw_commands = command_values
        commanded_target = relative_commands_to_target(raw_commands)

    elif command_source == "instrument_angles":
        # DOF runner: absolute instrument-angle commands.
        instrument_commands = command_values

        commanded_target = np.array([
            convert_instrument_angles_to_motor_targets(
                instrument_angles=instrument_commands[:, sample_index],
                gearbox_variant=gearbox_variant,
                instrument_config=instrument_config,
            )
            for sample_index in range(instrument_commands.shape[1])
        ], dtype=float).T

        # Convert absolute target changes into a command-like signal.
        raw_commands = np.zeros_like(commanded_target)
        raw_commands[:, 1:] = np.diff(
            commanded_target,
            axis=1,
        )
        ...
        raise RuntimeError(
            f"Unsupported command source: {command_source}"
        )

    # Both prediction target and measured response are relative to the
    # beginning of this trial.
    for motor_index in range(4):
        if positions.shape[1] > 0:
            positions[motor_index] -= positions[motor_index, 0]
...
def build_dataset(
    ...
    for segment in segments:
        ...
        if coupling_mode == "motor_only":
            ...
            # Tool/gearbox trials are labelled per DOF instead of per motor.
            # Select every motor that actually receives a command in the segment.
            _, raw_commands, commanded_target, _, _ = segment_to_arrays(
                segment,
                gearbox_variant=gearbox_variant,
                instrument_config=instrument_config,
            )
            motor_indices = [
                ...
                for motor_index in range(4)
                if (
                    np.any(np.abs(raw_commands[motor_index]) > 1e-5)
                    or np.ptp(commanded_target[motor_index]) > 1e-5
                )
            ]
...
def load_segments_from_files(file_paths, coupling_mode):
    all_segments = []
...
def train_motor_digital_twin(file_paths, output_dir, coupling_mode):
