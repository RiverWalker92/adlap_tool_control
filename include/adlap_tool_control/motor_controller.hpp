...
public:
  ...
  Motor(int magnets, float gear_ratio, int encoder_mode, int duty_cycle_percentage, int max_current,
        int emergency_current, bool reverse_direction, bool inverse_driven)
    : magnets(magnets)
  ...
  static Motor create(int magnets, float gear_ratio, int encoder_mode, int duty_cycle = 40, int max_current = 500,
                      int emergency_current = 1000, bool reverse_direction = true, bool inverse_driven = false)
  {
    return { magnets,     gear_ratio,        encoder_mode,      duty_cycle,
             max_current, emergency_current, reverse_direction, inverse_driven };
  }
...
public:
  ...
  const std::array<int, 4>& get_starting_positions() const
  {
    return starting_positions;
  }
  const std::array<int, 4>& get_target_positions() const
  {
    return target_positions_;
  }
  const std::array<bool, 4>& get_blocked_status() const
  {
    return stalled_;
  }
  const std::array<bool, 4>& get_maxed_status() const
  {
    return overcurrent_;
  }
