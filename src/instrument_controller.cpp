  ...
  for (;;)
  {
    ...
    switch (c)
    {
      ...
      case KEYCODE_U:
        ...
        // ofset articulation
        current_positions[3] -=
            50 * motor_controller_.get_pulses_per_degree(true);  // Add some extra to compensate for backlash
        ...
        RCLCPP_INFO(logger_, "Smoothed angles - roll: '%f', pitch: '%f', yaw: '%f', articulation: '%f'",
                    smoothed_tip_rotation_, smoothed_pitch_, smoothed_yaw_, smoothed_articulation_);
      ...
      case KEYCODE_0:
        ...
        {
          RCLCPP_WARN(logger_,
                      "Not ready for angle control yet, press 'I' to initialize motors and 'U' to update starting "
                      "positions");
          break;
        }
      ...
      case KEYCODE_5:
        ...
        motor_controller_.send_relative_motor_positions(0, motor_controller_.get_pulses_per_rotation(false),
                                                        motor_controller_.get_pulses_per_rotation(false), 0);
        break;
      case KEYCODE_6:
...
double InstrumentController::update_history_and_get_mean(std::deque<double>& history, double sample,
                                                         std::size_t max_size, double min_value, double max_value)
{
...
void InstrumentController::set_euler_angles(double roll, double pitch, double yaw, double articulation, bool verbose)
{
  ...
  smoothed_articulation_ =
      update_history_and_get_mean(articulation_history_, articulation, smoothing_factor_, 0, M_PI / 6);
  if (verbose)
    RCLCPP_DEBUG(logger_, "Smoothed angles - roll: '%f', pitch: '%f', yaw: '%f', articulation: '%f'",
                 smoothed_tip_rotation_, smoothed_pitch_, smoothed_yaw_, smoothed_articulation_);
  motor_controller_.send_motor_positions(calculate_motor_positions_from_euler_angles(smoothed_tip_rotation_,
                                                                                     smoothed_pitch_, smoothed_yaw_,
                                                                                     smoothed_articulation_, verbose),
                                         true);
}

void InstrumentController::set_joint_angles(double shaft_roll, double bend, double tip_rotation, double articulation,
                                            bool verbose)
{
  // Add to history + compute smoothed values (same helper called 4 times)
  smoothed_shaft_roll_ =
      update_history_and_get_mean(shaft_roll_history_, shaft_roll, smoothing_factor_, -INFINITY, INFINITY);
  smoothed_bend_ = update_history_and_get_mean(bend_history_, bend, smoothing_factor_, -M_PI / 4, M_PI / 4);
  smoothed_tip_rotation_ =
      update_history_and_get_mean(tip_rotation_history_, tip_rotation, smoothing_factor_, -M_PI, M_PI);
  smoothed_articulation_ =
      update_history_and_get_mean(articulation_history_, articulation, smoothing_factor_, 0, M_PI / 6);
  if (verbose)
    RCLCPP_DEBUG(logger_, "Smoothed angles - shaft_roll: '%f', bend: '%f', tip_rotation: '%f', articulation: '%f'",
                 smoothed_shaft_roll_, smoothed_bend_, smoothed_tip_rotation_, smoothed_articulation_);
  motor_controller_.send_motor_positions(
      calculate_motor_positions_from_joint_angles(smoothed_shaft_roll_, smoothed_bend_, smoothed_tip_rotation_,
                                                  smoothed_articulation_, verbose),
      true);
}
...
int InstrumentController::get_motor2_value_for_angle(double radians, bool verbose)
{
  ...
  int starting_difference =
      motor_controller_.get_starting_positions()[2] -
      motor_controller_.get_starting_positions()[1];  // Starting difference between motor 2 and motor 1, compensated
                                                      // for the initial offset found in setup
  // int current_difference = motor_controller_.get_positions()[2] - motor_controller_.get_positions()[1] -
  // starting_difference - 2 * bend_play_compensation_; // Current difference between motor 2 and motor 1, compensated
  // for play

  // Use real play to prevent incorrect position estimate if the motors havent reached target yet.
  int real_play =
      calculate_real_play(motor_controller_.get_positions());  // Calculate the real play based on the current motor
                                                               // positions and the current offset
  int current_difference = motor_controller_.get_positions()[2] - motor_controller_.get_positions()[1] -
                           starting_difference -
                           2 * real_play;  // Current difference between motor 2 and motor 1, compensated for play

  int wanted_difference =
...
std::array<int, 4> InstrumentController::calculate_motor_positions_from_euler_angles(double roll, double pitch,
                                                                                     double yaw, double articulation,
                                                                                     bool verbose)
{
...
std::array<int, 4> InstrumentController::calculate_motor_positions_from_joint_angles(double shaft_roll, double bend,
                                                                                     double tip_rotation,
                                                                                     double articulation, bool verbose)
{
  // Calculate the shortest rotation for shaft_roll
  // We need to choose the new shaft_roll to be the closest to the current position to prevent unnecessary full
  // rotations The motors are driven with absolute positions, so we need to keep track of the absolute shaft_roll
  ...
  int tip_rotation_pulses =
      -static_cast<int>(std::round(tip_rotation * motor_controller_.get_pulses_per_rotation(true) / (TWO_PI)));
  int articulation_pulses = static_cast<int>(
      std::round(ARTICULATION_FACTOR * articulation * motor_controller_.get_pulses_per_rotation(true) / (TWO_PI)));
  ...
  std::array<int, 4> new_positions = {
    ...
    static_cast<int>(
        std::round(starting_positions[2] + shaft_rotation_pulses + m2_bend_pulses + bend_play_compensation_)),
    static_cast<int>(std::round(starting_positions[3] + tip_rotation_pulses + articulation_pulses))
  };
  return new_positions;
}

// TODO: make this trigger on each recieved message in motor thread, otherwise we cannot reliably use m1_m2_offset_
// variable. Now kinda fixed by calling this each time we recieve a new command in the node during motor position
// calculation.
int InstrumentController::calculate_real_play(const std::array<int, 4>& current_positions)
{
  ...
  int starting_diff =
      starting_positions[2] - starting_positions[1];  // Starting difference between motor 2 and motor 1, compensated
                                                      // for the initial offset found in setup

  int diff = current_positions[2] - current_positions[1] -
             starting_diff;  // Current difference between motor 2 and motor 1, not yet compensated for play
  if (diff > 0)
  {
...
std::array<double, 4> InstrumentController::joint_angles_from_motors(const std::array<int, 4>& current_positions)
{
  ...
  double tip_rotation = (static_cast<double>(starting_positions[0] - current_positions[0]) * TWO_PI /
                         motor_controller_.get_pulses_per_rotation(true));  // Invert the roll calculation
  double articulation_angle = (static_cast<double>(current_positions[3] - starting_positions[3] - current_positions[0] +
                                                   starting_positions[0]) *
                               TWO_PI / motor_controller_.get_pulses_per_rotation(true)) /
                              ARTICULATION_FACTOR;  // Invert the articulation calculation

  int real_play = calculate_real_play(current_positions);  // Calculate the real play based on the current motor
                                                           // positions and the current offset
