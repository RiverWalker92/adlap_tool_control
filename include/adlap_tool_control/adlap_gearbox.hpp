...
public:
  ...
  // Constructor
  Gearbox(MotorController& motor_controller, rclcpp::Logger logger)
    : motor_controller(motor_controller), logger_(logger)
  {
  }
                           ...
                           float upper_motor_factor = 25.0f / 15.0f, int lower_motors_play = 15)
  {
    ...
    gearbox.lower_motor_start_offset_ =
        20 * gearbox.get_pulses_per_degree(1);  // Add some extra to compensate for backlash
    gearbox.gripper_start_offset_ = 200 * gearbox.get_pulses_per_degree(3);  // Move it away from the hard stop

    RCLCPP_INFO(logger, "Initialized Gearbox with upper_motor_factor=%.2f and lower_motors_play=%d degrees",
                upper_motor_factor, lower_motors_play);
    return gearbox;
  }
  ...
  int get_pulses_lower_motors_play() const
  {
    return static_cast<int>(std::lround(lower_motors_play * get_pulses_per_degree(1)));
  }  // Using motor 1, since it doesn't matter which of the lower motors we use for this compensation
