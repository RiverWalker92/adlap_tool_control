...
void MotorController::send_motor_configuration(int motor_index, bool verbose)
{
  ...
  std::string message =
      "config " + std::to_string(motor_index) + ", " + std::to_string(0) +
      ", " +                                                              // Min duty cycle (fix with proper value)
      std::to_string(motors[motor_index].duty_cycle_percentage) + ", " +  // Max duty cycle
      std::to_string(motors[motor_index].max_current) + ", " + std::to_string(motors[motor_index].emergency_current) +
      ", " + std::to_string(static_cast<int>(motors[motor_index].gear_ratio)) + ", " +
      std::to_string(motors[motor_index].magnets) + ", " + std::to_string(motors[motor_index].encoder_mode) + ", " +
      std::to_string(reverse_int) + ", " + std::to_string(inverse_driven_int) + "\n";
  if (verbose)
  {
    std::string log_message = message.substr(0, message.size() - 1);
